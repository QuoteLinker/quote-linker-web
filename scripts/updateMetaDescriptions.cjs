...
function generateMetaDescription(title, existingContent) {
  if (API_PROVIDER === 'openai') {
    ...
    return Promise.reject(new Error('Unsupported API provider: ' + API_PROVIDER));
  }
}
...
function getMdxFiles(dir) {
  ...
  return readDir(dir)
    .then(function(files) {
      // Process each file sequentially using reduce
          ...
          return stat(filePath)
            .then(function(stats) {
              if (stats.isDirectory()) {
                return getMdxFiles(filePath).then(function(subFiles) {
...
function processBatches(files, force) {
  ...
  function processBatch(startIndex) {
    ...
    return batch.reduce(function(promise, file, index) {
      return promise
        .then(function() {
    ...
    .then(function() {
      ...
      if (endIndex < files.length) {
        ...
        return sleep(RATE_LIMIT_DELAY * 2);
      }
    })
    .then(function() {
      // Process next batch
...
function main() {
  ...
  // Get all MDX files
  return getMdxFiles(CONTENT_DIR)
    .then(function(files) {
