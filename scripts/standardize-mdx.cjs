...
function addMissingFrontmatter(content) {
  var parsed = matter(content);
  var data = parsed.data;
  var mdxContent = parsed.content;
  var updated = false;
  
  // Extract title root for slugs that don't have additional frontmatter
  var titleRoot = data.title || '';
  if (titleRoot.toLowerCase().startsWith('learn: ')) {
    titleRoot = titleRoot.substring(7); // Remove "Learn: " prefix
  }
  
  // Add missing frontmatter fields
  if (!data.description) {
    data.description = "A comprehensive guide to understanding " + titleRoot.toLowerCase() + 
                      " options, coverage types, and how to choose the right policy for your needs.";
    updated = true;
  }
  
  if (!data.coverImage) {
    // Generate a default cover image path based on slug or title
    var slug = data.slug || titleRoot.toLowerCase().replace(/\s+/g, '-');
    data.coverImage = "/images/learn/" + slug + "-cover.jpg";
    updated = true;
  }
  
  if (!data.readingTime) {
    // Default reading time
    data.readingTime = "8 min read";
    updated = true;
  }
  
  if (!data.category) {
    // Try to extract category from title or use a default
    var category = "Insurance";
    
    if (/auto|car/i.test(titleRoot)) {
      category = "Auto Insurance";
    } else if (/home|homeowners|property/i.test(titleRoot)) {
      category = "Home Insurance";
    } else if (/life/i.test(titleRoot)) {
      category = "Life Insurance";
    } else if (/health/i.test(titleRoot)) {
      category = "Health Insurance";
    } else if (/disability/i.test(titleRoot)) {
  ...
  if (updated) {
    return matter.stringify(mdxContent, data);
  }
...
function lintMDXFile(filePath) {
  ...
  var hasMissingFrontmatter = false;
  for (var n = 0; n < issues.length; n++) {
    if (issues[n].includes("Missing required frontmatter field:")) {
      hasMissingFrontmatter = true;
      break;
    }
  }
  
  if (hasMissingFrontmatter) {
    updatedContent = addMissingFrontmatter(updatedContent);
  }
  ...
  return readDir(dirPath, { withFileTypes: true }).then(function(entries) {
    ...
    for (var i = 0; i < entries.length; i++) {
      ...
      if (entry.isDirectory()) {
        ...
        promises.push(lintMDXFile(entryPath).then(function(result) {
          return [result];
        }));
      }
    }
